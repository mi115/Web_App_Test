...
def test_v_register_view_new_user():
    ...
    # Arrange
    from first_site.views.account_views import register_post
    form_data = {
        'name': 'Michael',
        'email': 'michael@talkpython.fm',
        'password': 'a' * 6
    ...
    target = 'services.user_service.find_user_by_email'
    find_user = unittest.mock.patch(target=target, return_value=None)
    target = 'first_site.services.user_service.create_user'
    create_user = unittest.mock.patch(target=target, return_value=User())
...
def test_int_account_home_with_login(client):
    target = 'services.user_service.find_user_by_id'
    test_user = User(name='Matthew', email='mattmills2110@gmail.com')
